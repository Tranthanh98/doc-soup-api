CREATE PROCEDURE [dbo].[sel_user_statistic_of_company]
...
CREATE PROCEDURE [dbo].[sel_statictis_visit_by_num_of_day]
    @accountId varchar(36),
    @companyId UNIQUEIDENTIFIER,
    @numOfDay INTEGER
AS
BEGIN
    SELECT sum(ls.visit) AS visits, CONVERT(date, ls.viewed_at) as viewedAt
    FROM link_statistic ls LEFT JOIN link l on l.id = ls.link_id
    WHERE l.created_by = @accountId
    and l.company_id = @companyId
    and ls.viewed_at BETWEEN DateAdd(DD, -@numOfDay,GETDATE() ) and GETDATE()
    GROUP BY CONVERT(date, ls.viewed_at)
END
GO