...
Select d.id as id, d.name as name, d.is_active as isActive, d.account_id as accountId, d.company_id as companyId, d.view_type as viewType,
       CONVERT(DATETIME2, d.created_date, 1) as createdDate, CONVERT(DATETIME2, d.modified_date, 1), STRING_AGG(ISNULL(la.name, ' '), ',') as sharedWithAccount, (a.first_name + ' ' + a.last_name) as owner,
...
SELECT d.id as id, d.name as name, d.is_active as isActive, d.account_id as accountId, d.company_id as companyId, d.view_type as viewType,
       CONVERT(DATETIME2, d.created_date, 1) as createdDate,  CONVERT(DATETIME2, d.modified_date, 1) as modifiedDate, STRING_AGG(ISNULL(la.name, ' '), ',') as sharedWithAccount, (a.first_name + ' ' + a.last_name) as owner,
       CASE WHEN EXISTS (SELECT id FROM link l WITH (NOLOCK) WHERE l.ref_id = d.id and l.document_id IS NULL and l.status=0) THEN CONVERT(BIT, 0) ELSE CONVERT(BIT, 1) END AS disabledAllLink
from data_room d WITH (NOLOCK)
