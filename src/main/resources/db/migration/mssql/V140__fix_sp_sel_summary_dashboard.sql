...
select N'docs'      as category,
       ...
       (select m.newDocs
        from (
                 select count(a.id) as newDocs
                 from file_entity AS a WITH(NOLOCK)
                 where a.created_date >= cast ( GETUTCDATE() as date ))
                 as m)
                    as newValue
...
select N'monthlyEarnings'   as category,
       ...
       (select m.newEarning
        from (
                 select sum(a.total_amount) as newEarning
                 from payment_history AS a WITH(NOLOCK)
                 where a.status = 'PAYMENT_PAID'
                   and cast (a.created_date as date) BETWEEN DATEADD(month
                     , DATEDIFF(month
                     , 0
...
where cast (d1.created_date as date) BETWEEN DATEADD(month
    , DATEDIFF(month
    , 0
    , cast (DATEADD(mm
    , -1
