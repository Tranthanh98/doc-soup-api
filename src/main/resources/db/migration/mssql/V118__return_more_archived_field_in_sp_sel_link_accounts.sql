...
IF @archived IS NULL
    ...
    SELECT la.id, la.name, la.created_by, lc.contributors as contributors, CONVERT(DATETIME2, ISNULL(MAX(s.viewed_at), ISNULL(MAX(l.created_date), ISNULL(MAX(la.modified_by), MAX(la.created_date)))), 1) as lastActivity, (CASE WHEN SUM(s.visit) > 0 THEN SUM(s.visit) ELSE 0 END) as totalVisit, lav.totalVisitor as totalVisitor, SUM(s.duration) as totalDuration, la.archived as archived
    FROM link_accounts la WITH (NOLOCK)
