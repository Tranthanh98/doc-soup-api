ALTER TABLE page_statistic
    ADD visit int NOT NULL DEFAULT (1)
GO
...
BEGIN
    ...
    SELECT A.topPage,A.topPageVisits,A.topPageDuration,B.visits,C.numOfViewedPage,D.totalPage FROM
        (
            select top 1 1 AS AID, p.page as topPage, SUM(p.visit) as topPageVisits, SUM(p.duration) as topPageDuration
            ...
            LEFT JOIN
        (
            select 1 AS BID, sum(s.visit) as visits from link_statistic s
                                                            INNER JOIN link l
                                                                       on s.link_id = l.id
            WHERE l.ref_id=@fileId and l.document_id is not null
