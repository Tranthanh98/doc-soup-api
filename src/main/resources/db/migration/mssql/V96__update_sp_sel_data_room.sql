...
Select d.id as id, d.name as name, d.is_active as isActive, d.account_id as accountId, d.company_id as companyId, d.view_type as viewType,
       ...
       CASE WHEN EXISTS (SELECT id FROM link l WITH (NOLOCK) WHERE l.ref_id = d.id and l.document_id IS NULL and l.disabled=0) THEN CONVERT(BIT, 0) ELSE CONVERT(BIT, 1) END AS disabledAllLink
