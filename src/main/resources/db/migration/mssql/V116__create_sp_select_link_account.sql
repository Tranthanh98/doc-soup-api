GO
CREATE PROCEDURE [dbo].[sel_link_account]
...
DECLARE @linkAccountWithTotalVisitorDuplicate TABLE (
    linkAccountId BIGINT,
    totalVisitor BIGINT
)

INSERT INTO @linkAccountWithTotalVisitorDuplicate
...
DECLARE @linkAccountWithTotalVisitorDisctint TABLE (
    linkAccountId BIGINT,
    totalVisitor BIGINT
)

INSERT INTO @linkAccountWithTotalVisitorDisctint
SELECT a.linkAccountId, SUM(a.totalVisitor)
FROM @linkAccountWithTotalVisitorDuplicate a
GROUP BY a.linkAccountId
...
IF @archived IS NULL
    ...
    SELECT la.id, la.name, la.created_by, lc.contributors as contributors, CONVERT(DATETIME2, ISNULL(MAX(s.viewed_at), ISNULL(MAX(l.created_date), ISNULL(MAX(la.modified_by), MAX(la.created_date)))), 1) as lastActivity, (CASE WHEN SUM(s.visit) > 0 THEN SUM(s.visit) ELSE 0 END) as totalVisit, lav.totalVisitor as totalVisitor, SUM(s.duration) as totalDuration
    FROM link_accounts la WITH (NOLOCK)
    LEFT JOIN link l WITH (NOLOCK)
    ON l.link_accounts_id = la.id
    LEFT JOIN link_statistic s WITH (NOLOCK)
    ON s.link_id = l.id
    LEFT JOIN @linkAccountWithTotalVisitorDisctint lav
    ON lav.linkAccountId = la.id
    LEFT JOIN @linkCreatorTable lc
    ON lc.linkAccountId = la.id
    WHERE la.company_id = @companyId
    GROUP BY la.name, la.id, la.created_by, lc.contributors, lav.totalVisitor
ELSE
    INSERT INTO @archivedResult
    SELECT la.id, la.name, la.created_by, lc.contributors as contributors, CONVERT(DATETIME2, ISNULL(MAX(s.viewed_at), ISNULL(MAX(l.created_date), ISNULL(MAX(la.modified_by), MAX(la.created_date)))), 1) as lastActivity, (CASE WHEN SUM(s.visit) > 0 THEN SUM(s.visit) ELSE 0 END) as totalVisit, lav.totalVisitor as totalVisitor, SUM(s.duration) as totalDuration
    FROM link_accounts la WITH (NOLOCK)
...
IF @mode = 'personal'
    INSERT INTO @result
    SELECT sr.id, sr.name, sr.contributors, sr.lastActivity, sr.totalVisit, sr.totalVisitor, sr.totalDuration
...
SELECT r.id id, r.name as name, r.contributors as contributors, r.lastActivity as lastActivity, r.totalVisit as totalVisit, r.totalVisitor as totalVisitor, r.totalDuration as totalDuration
FROM @result r
