IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = 'sel_contact_with_link_model_by_company_id')
DROP PROCEDURE [dbo].[sel_contact_with_link_model_by_company_id]
GO

IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = 'sel_contact_with_link_model_by_companyId_archived_is_false')
DROP PROCEDURE [dbo].[sel_contact_with_link_model_by_companyId_archived_is_false]
GO

GO
CREATE PROCEDURE [dbo].[sel_contact]
...
left join (
    SELECT distinct c.id as contacId, la.name as linkName
    FROM contact c WITH (NOLOCK)
