...
import logixtek.docsoup.api.features.share.domainevents.sendemail.SendEmailRemindInvitationDomainEvent;
...
@Component("SendEmailRemindInvitationDomainEventHandler")
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class SendEmailRemindInvitationDomainEventHandler implements Notification.Handler<SendEmailRemindInvitationDomainEvent> {
    ...
    @Override
    public void handle(SendEmailRemindInvitationDomainEvent notification) {
        ...
        if(companyUserOption.isPresent()) {
            ...
            if(sender != null && companyOption.isPresent()) {
                ...
                var subject = "Join " + companyOption.get().getName() + " on DocSoup";
                var senderFullName = "";

                var htmlTemplate = "templates/reminderUserJoinToCompanyTemplate.html";
